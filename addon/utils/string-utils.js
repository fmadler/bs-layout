...
export function extractParams(s) {
  ...
  if (s!==undefined && s!=null) {
    ...
    while (end > 0) {
      ...
      if (end>start) { //end delimiter '}' can be for other purpose than wrapping a param (especially in json)
        let v = s.substring(
          start+ 2, //start delimiter length is 2
          end
        );
        if (ret.filter((p)=>p.name===v).length===0) {
          ret.push({label:v,name:v,value:"",currentValue:startDelimiter+v+endDelimiter});
        }
      }
